...
def getDataLabelledDocuments(
    ...
    """
    Parse all Araucaria documents and return document-level train/test splits.
    Unlike getDataLabelledSentences, this returns lists of documents (lists of sentences)
    to preserve document boundaries for context features.

    Returns
    -------
    train_docs, train_labels, test_docs, test_labels
        Lists of documents, where each document is a list of sentences (or integer labels).
    """
    json_files = sorted(glob.glob(os.path.join(data_dir, "nodeset*.json")))
    ...
    print(f"[araucaria] Documents Loaded: {len(train_docs)} train, {len(test_docs)} test")
    print(f"[araucaria] Total train sentences: {total_train_sents} ({total_train_claims} claims)")
    print(f"[araucaria] Total test sentences: {total_test_sents} ({total_test_claims} claims)")
